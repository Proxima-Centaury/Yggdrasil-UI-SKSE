...
void SharedMenuManager::SKSEGetMenu(const RE::FxDelegateArgs& args) {
	...
	if(std::strcmp(ui, "STARTMENU") == 0) {
		...
		if(std::strcmp(section, "MAINMENU") == 0) {
			...
			menu.addItem(YGGDRASIL::MenuItem(false, "New", std::nullopt, false, "YGUI_STARTMENU_MAIN_MENU_MENU_ITEM_NEW_GAME"));
			menu.addItem(YGGDRASIL::MenuItem(false, "Load", std::nullopt, false, "YGUI_STARTMENU_MAIN_MENU_MENU_ITEM_LOAD_GAME"));
			...
			menu.addItem(YGGDRASIL::MenuItem(false, "Quit", std::nullopt, false, "YGUI_STARTMENU_MAIN_MENU_MENU_ITEM_QUIT_GAME"));
